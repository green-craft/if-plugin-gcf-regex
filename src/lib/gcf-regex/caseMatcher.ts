...
const path_to_instance_text = path.join(__dirname, 'instanceTypes.txt');
const fs = require('fs');
function readFile(filename: string): any[] {
  ...
  try {
    ...
    if (!data) {
      throw new Error(
        'Failed to read instance types, the data I have is: '.concat(data, '.')
      );
    }
    console.log('Retrieved instance types');
    const instanceList = data.split('\n');
    // remove duplicates
    return [...new Set(instanceList)];
  } catch (error) {
    console.error('Error reading instance type list: '.concat(String(error)));
    throw Error(
      `Could not retrieve instance type file using node fs. Path is ${path_to_instance_text}`
    );
  }
}
function matchTypes(toCompare: string, instanceList: string[]): string {
  ...
  for (let i = 0; i < instanceList.length; i++) {
    const upperToCompare = toCompare.toUpperCase();
  ...
  if (matched_result === '') {
    throw Error(
      `No matching instance type found for instance type ${toCompare}`
    );
  }
...
export default function caseMatch(toCompare: string) {
  console.info(
    'Entering caseMatch function...\n        Path to file: '
      .concat(path_to_instance_text, ',\n        String to compare: ')
      .concat(toCompare)
  );
  const res = matchTypes(toCompare, readFile(path_to_instance_text));
